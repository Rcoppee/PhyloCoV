...
list_region = unique(metadata$continent_wave)
...
for(replicat in 1:n_replicates){
	...
	if(exists("final_data")){
		...
		final_data = data.frame(continent_wave=character(), total=integer())
		final_data = meta2
	}
}
...
metadata3 = metadata2 %>%
	group_by(continent_wave, week) %>%
	summarize(total=n())

#update North America and South America names
for(i in 1:nrow(metadata3)){
	if(metadata3$continent_wave[i]=="North.America"){
		metadata3$continent_wave[i]="North America"
	}else if(metadata3$continent_wave[i]=="South.America"){
		metadata3$continent_wave[i]="South America"
	}
}
...
for(i in 1:nrow(epidemio_fix2)){
	for(j in 1:nrow(metadata3)){
		if(epidemio_fix2$week[i]==metadata3$week[j]){
			if(epidemio_fix2$continent[i]==metadata3$continent_wave[j]){
