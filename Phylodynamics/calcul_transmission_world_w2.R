...
setwd("F:/phylocov/DEFINITIVE/World_W2")
...
for(continent in list_continent){
  ...
  #we complete every lacking data for each week, region and replicate in 2020
  for(a_week in 1:52){
...
mean_transmission$region %<>% factor(levels=c("Europe","North.America", "South.America", "Asia", "France","Africa","Oceania"))

#stacked area chart
ggplot(mean_transmission, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=region), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c("#538DCA", "#1A7E41","#FEDA27",  "#5F4197",  "#54C538", "#EF3437","#FF8421"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
#we order the data for the plot
mean_transmission$region %<>% factor(levels=c("Europe","North.America", "Asia", "South.America", "France", "Africa", "Oceania"))

#stacked area chart
ggplot(mean_transmission, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=region), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c( "#538DCA", "#1A7E41","#5F4197", "#FEDA27", "#54C538", "#EF3437", "#FF8421"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
#we order the data for the plot
mean_df_inter_SYM3$parent_loc %<>% factor(levels=c("Europe", "North.America", "Asia", "South.America", "Africa", "Oceania"))
...
#we order the data for the plot
mean_df_inter_SYM3$node_loc %<>% factor(levels=c("Europe", "Africa", "North.America", "Asia", "South.America", "Oceania"))

#stacked area chart
ggplot(mean_df_inter_SYM3, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=node_loc), size=.5, colour="black", position="stack")+
  #scale_fill_manual(values = c("#FF8421", "#FEDA27", "#5F4197", "#1A7E41","#EF3437", "#538DCA"))+
  scale_fill_manual(values = c("#538DCA", "#EF3437", "#1A7E41", "#5F4197","#FEDA27", "#FF8421"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
introduction = ggplot(data = introduced_france3, aes(x=type, y=freq, fill=final_clade))+
  ...
  geom_label(aes(label = final_clade),
             ...
             size = 3,
             colour = 'white')+
  scale_fill_manual(values = c("#00545c","#a05195", "#f95d6a",
                               "#ffa600","#ff6000", "#5751a0"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
exportation = ggplot(data = france_export3, aes(x=type, y=freq, fill=final_clade))+
  geom_bar(stat="identity")+
  scale_fill_manual(values = c("#f95d6a", "#ff6000"))+
  geom_label(aes(label = final_clade),
             position = position_stack(vjust = 0.5),
             size = 3,
             colour = 'white')+
