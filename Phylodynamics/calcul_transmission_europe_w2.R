...
setwd("F:/phylocov/DEFINITIVE/Europe_W2")
...
#remove the first transmissions originating from China
circos_SYM = circos_SYM[circos_SYM$parent_loc!="China",]
circos_SYM = circos_SYM[circos_SYM$node_loc!="China",]
...
for(continent in list_continent){
  ...
  mean_df_inter_SYM3 <- mean_df_inter_SYM2 %>%
    group_by(week, year) %>%
    summarise(total2=(sum(total)/n_replicat))
...
mean_transmission$region %<>% factor(levels=c("Germany","United.Kingdom", "Italy", "France", "Poland", "Belgium",
                                              "Netherlands", "Spain", "Sweden",  "Romania", "Russia"))
#stacked area chart
ggplot(mean_transmission, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=region), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c("#FD832F",  "#ED313C",  "#558EC8",
                               "#57C540", "#813008", "#FDD93B",
                               "#872BBA","#10791E", "#867017",
                               "#C485E6", "#16999F"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
...
#we order the data for the plot
mean_transmission$region %<>% factor(levels=c("Russia", "Spain", "France", "Germany",
                                              "Italy", "Poland", "United.Kingdom", "Belgium",
                                              "Romania", "Netherlands", "Sweden"))
#stacked area chart
ggplot(mean_transmission, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=region), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c("#16999F", "#10791E",  "#57C540",  
                               "#FD832F", "#558EC8", "#813008",
                               "#ED313C", "#FDD93B", "#C485E6",
                               "#872BBA",  "#867017"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
...
FR_SYM_intro = ggplot(France_SYM, aes(x=parent_loc, y=a_sum, fill=parent_loc)) + 
  ...
  scale_fill_manual(values = c("#FDD93B", "#000000", "#FD832F", "#558EC8","#872BBA", "#813008", "#C485E6",
                               "#16999F", "#10791E", "#867017", "#ED313C"))+
  geom_line()+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
...
locations = c("Belgium","Germany","Italy",
              "Netherlands","Poland","Romania","Russia",
              "Spain","Sweden","United.Kingdom")
...
#we remove China transmissions to retain at the Europe scale
mean_df_inter_SYM3 = mean_df_inter_SYM3[mean_df_inter_SYM3$parent_loc!="China",]

#we order the data for the plot
mean_df_inter_SYM3$parent_loc %<>% factor(levels=c("Russia", "Spain", "Italy", "Germany", "Poland",
                                                   "United.Kingdom", "Belgium", "Romania", "Netherlands", "Sweden"))
#stacked area chart
ggplot(mean_df_inter_SYM3, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=parent_loc), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c("#16999f", "#10791e", "#558ec8", "#fd832f", "#813008",
                               "#ed313c", "#fdd93b", "#c485e6", "#872bba", "#867017"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
#we order the data for the plot
mean_df_inter_SYM3$node_loc %<>% factor(levels=c("Italy", "Germany", "Belgium", "United.Kingdom", "Netherlands",
                                                 "Sweden", "Spain", "Poland", "Romania", "Russia"))
#stacked area chart
ggplot(mean_df_inter_SYM3, aes(x=week_2, y=total2)) + 
  geom_area(aes(fill=node_loc), size=.5, colour="black", position="stack")+
  scale_fill_manual(values = c("#558ec8", "#fd832f", "#fdd93b", "#ed313c","#872bba",
                               "#867017", "#10791e", "#813008", "#c485e6","#16999f"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
        panel.grid.major = element_blank(),
...
introduction = ggplot(data = introduced_france3, aes(x=type, y=freq, fill=final_clade))+
  ...
  geom_label(aes(label = final_clade),
             ...
             size = 3,
             colour = 'white')+
  scale_fill_manual(values = c("#31af84","#d10f4a", "#ff6000",
                               "#ffa600", "#00545c","#f95d6a",
                               "#a05195", "#5751a0"))+
  theme(axis.line.x = element_line(size = 1, colour = "black"),
        axis.line.y = element_line(size = 1, colour = "black"),
        axis.line = element_line(size=1, colour = "black"),
...
exportation = ggplot(data = france_export3, aes(x=type, y=freq, fill=final_clade))+
  geom_bar(stat="identity")+
  scale_fill_manual(values = c("#00545c", "#5751a0",
                               "#f95d6a", "#ff6000"))+
  geom_label(aes(label = final_clade),
             position = position_stack(vjust = 0.5),
             size = 3,
